...
def load_np_image(image_path):
    ...
    exif_dict = piexif.load(normal_image.info["exif"])
    ...
    return np_image, exif_bytes


def save_np_image(image, image_path, exif):
...
class Anonymizer:
    ...
    def anonymize_images(self, input_path, output_path, detection_thresholds, file_types, write_json):
        ...
        for input_image_path in tqdm(files):
            ...
            # Anonymize image
            # May 20, 2022 - Added exif_data for saving image EXIF to anonymized image
            ...
            save_np_image(image=anonymized_image, image_path=str(output_image_path), exif=exif_data)
            if write_json:
